...
void computeMembers(int* numOfMembers, Cluster* currentClusters, Point* allPoints,int numOfClusters, int numOfPoints)
{
	for (int c = 0; c< numOfClusters; c++)
	{
		...
		for (int j = 0; j < numOfPoints - 1; j++)
		{
			...
			{
				if (allPoints[j].getNearestCluster() == c && allPoints[k].getNearestCluster() == c)
				{
					numOfMembers[c]++;
				}
			}
...
int main(int argc, char *argv[])
{
	...
	const string IN_FILE = "C:\\Users\\afeka.ACADEMIC\\Desktop\\KMeansCuda OMP Project\\points100.txt";
	const string OUT_FILE = "C:\\Users\\afeka.ACADEMIC\\Desktop\\KMeansCuda OMP Project\\results.txt";
	const int MASTER = 0;
...
#pragma omp parallel for shared(currentClusters, allPoints) private(k)
			...
			int* numOfMembers = new int [numOfClusters];
			computeMembers(numOfMembers, currentClusters, allPoints, numOfClusters, numOfPoints);
			for (int i = 0 ; i<numOfClusters; i++)
			{
				cout << "members[" << i << "] = " << numOfMembers[i] << endl;
			}
			/////////////
